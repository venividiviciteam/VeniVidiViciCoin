...
unsigned int nStakeMaxAge = 24 * 3600 * 24 * 100;           //100 days
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight <= 32)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
    
    else if (pindexBest->nHeight > 5000 && pindexBest->nHeight < 7551)
    {
      nSubsidy = 100 * COIN;
      return nSubsidy + nFees;
    }
    else if (pindexBest->nHeight > 7550)
    {
      nSubsidy = 20 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VeniVidiVici", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "5/25";
        CTransaction txNew;
        txNew.nTime = 1401040330;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1401040330;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1081010;
		if(fTestNet)
        {
            block.nNonce   = 1891010;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x89ef8da3dac42480e2e8ef1cc995f820bc183ad2c0a445cbcb356ceccd37b604"));
        block.print();
